...
class BGRU(nn.Module):
    ...
    def __init__(self):
        ...
        self.embedding = nn.Embedding(LETTERS, BGRU.embedding_dim)
        self.gru = nn.GRU(BGRU.embedding_dim, BGRU.hidden_dim, batch_first=True)  # batch_first=False par défaut
        self.linear = nn.Linear(BGRU.hidden_dim, 1)


    def get_hidden_size(self):
        return BGRU.hidden_dim
    ...
    def step(self, hidden_state, input_char):
        ...
        _, next_hidden_state = self.gru(embedded, hidden_state.to(DEVICE))
    ...
    def step_batch(self, hidden_state, input_char): #TODO à checker
            ...
            _, next_hidden_state = self.gru(embedded, hs)
            return next_hidden_state
        ...
        with torch.no_grad():
            for X, _ in dataloader:
                ...
                for i in range(X.size(1)):
                    ...
                    _, hidden_state = self.gru(embedded, hidden_state) # unsqueeze(1) permet d'avoir une dimension [1024,1] au lieu de [1024]
                    hidden_states.append(hidden_state)
        return torch.cat(hidden_states, dim=1)
    
    def train(self, dataloader):
        print("Début de l'entraînement du BGRU...")
        a = ""
        optimizer = optim.Adam(self.parameters(), lr=0.001)  # Adam optimizer
        # Boucle d'entraînement
        for epoch in range(BGRU.epochs):
            total_loss = 0.0
            cpt = 0
            ...
            if (epoch + 1) % 10 == 0:
                ...
                print(f'Epoch [{epoch+1}/{BGRU.epochs}], Loss: {avg_loss:.4f}')
            ...
            avg_loss = total_loss / cpt
            a += f"{avg_loss};"
        with open("loss_bgru.csv", "a") as f:
            f.write(a + "\n") """
    ...
    def get_hyperparameters(self):
        return {
            "embedding_dim": BGRU.embedding_dim,
            "hidden_dim": BGRU.hidden_dim,
            "epochs": BGRU.epochs
        }
...
if __name__ == "__main__":
    ...
    model_pth = "models/GRU_bkmu.500.pth"
    batch_size = 1024
