...
class Automaton:
    ...
    def __init__(self, model, Sigma, state_number, dataloader, init_build="brute"): #model is a RNN with some stuff
        device = get_device()
        self.method = model.method
        hsize = model.get_hidden_size()
        H = None

        all_hs = model.get_all_hidden_states_gpu(dataloader)

        H = all_hs.detach().cpu().numpy()
        ...
        kmeans = faiss.Kmeans(d=hsize, k=state_number, niter=20, verbose=True, gpu=True)
        kmeans.train(H)
        if init_build == "brute":
        ...
        if init_build == "pred":
            _, node_pred = kmeans.index.search(model.get_default_hidden_state().detach().cpu().numpy().reshape(-1, hsize), 1)[0]
        ...
        for q in Q:
            ...
            _, p = kmeans.index.search(k.squeeze(0).detach().cpu().numpy(), 1) #the corresponding state
            p = p.flatten() # from 2D to 1D ([[1], [2], [3]] -> [1, 2, 3])
            for j in Sigma:
    ...
    def qualite_classe(self, H, centers, kmeans): #TODO: à revoir pour adapter à chaque tête de classification
        ...
        H = H[1:,:]
        _, pred = kmeans.index.search(H, 1)
